...
$(function () { // DOM ready
  ...
  $('.bt_new_game').live('click', function () {
    ...
    _gaq.push(['_trackEvent', 'Poke Games', 'Jugar denuevo', 'Jugar denuevo']);
    init_game();
  });
  ...
  if($('#android_link').length) {
    ...
    if(ua.indexOf('android') > -1) {
     // $('#android_link').attr('href', 'market://details?id=com.phonegap.');
    }
...
function init_game() {
  ...
  // tracking Google Analytics
  _gaq.push(['_trackEvent', 'Poke-Games', 'Iniciar', 'Iniciar']);
  ...
  if (chaine_trouvee) {
    ...
    if ($icone == undefined && $binome == undefined) {
      if (test_possible_move()) {
        ...
        // tracking Google Analytics
        _gaq.push(['_trackEvent', 'Poke-Games', 'Game over', 'Game over', score]);
        ...
        //$('#zone_message').append('<a target="_blank" href="http://www.baptistebrunet.com/games/" class="button">Mas juegos</a>');
        $('#zone_message').append('<div class="button bt_new_game">Jugar denuevo</div>');
      ...
      while(ligne+i < NB_COLONNES && tab_icones[ligne+i][colonne] == num_icone) {
        ...
        if(i == 1 && multiplier%5 == 0){
          ...
          if(i == 1 && suite_verti == 4){
            ...
            $('#zone_message').append('<div class="fire">FUEGO!</div>');
          } else {
...
function explosion(ligne, colonne) {
  ...
  $('#zone_message').append('<div class="fire">Grandioso!</div>');
  setTimeout(function () {
    $explosion.remove();
  }, 600);
...
function destroy_line_column(ligne, colonne) {
  $('#zone_message').append('<div class="star">GRANDIOSO!</div>');
  for (var i = 0; i < NB_LIGNES; i++) {
...
function test_possible_move() {
  ...
  if(nb_possible_moves <= 3) {
    if(nb_possible_moves <= 1) {
      $('#moves').addClass('critical').html(nb_possible_moves + '<br>Int.');
    } else {
      $('#moves').removeClass('critical').html(nb_possible_moves + '<br>Int.');
    }
  } else {
